...
// Rute berdasarkan Google My Maps UNDIP
const ROUTE_1_PATH = [
    // === Joging Track → Baskoro (Jl Prof Eko Budiharjo, ke utara) ===
    [-7.0553551, 110.4392964], // Halte Joging Track
    [-7.0544000, 110.4392500],
    [-7.0536302, 110.4391998], // Halte Baskoro

    // === Baskoro → FISIP Hukum (belok kiri, ke barat laut) ===
    [-7.0528000, 110.4389000],
    [-7.0520000, 110.4383000],
    [-7.0514000, 110.4377000],
    [-7.0509097, 110.4371935], // Halte FISIP Hukum

    // === FISIP Hukum → FIB Vokasi (ke barat) ===
    [-7.0506000, 110.4366000],
    [-7.0504200, 110.4360992], // Halte FIB Vokasi

    // === FIB Vokasi → Widpur (ke timur laut, Jl Antonius Suroyo) ===
    [-7.0501000, 110.4366000],
    [-7.0499000, 110.4374000],
    [-7.0498503, 110.4384971], // Halte Widpur

    // === Widpur → Samwa (Jl Prof Soedarto, ke timur) ===
    [-7.0493000, 110.4390000],
    [-7.0490000, 110.4396000],
    [-7.0487483, 110.4402190], // Halte Samwa

    // === Samwa → Psiko (Jl Prof Mr Sunario, ke barat laut) ===
    [-7.0481000, 110.4398000],
    [-7.0477000, 110.4393000],
    [-7.0473000, 110.4389000],
    [-7.0471884, 110.4387009], // Halte Psiko

    // === Psiko → FEB (Jl Prof Mr Sunario balik ke timur) ===
    [-7.0473000, 110.4392000],
    [-7.0475000, 110.4400000],
    [-7.0476915, 110.4410277], // Halte FEB

    // === FEB → FKM (Jl Prof Jacub Rais, ke timur laut) ===
    [-7.0482000, 110.4416000],
    [-7.0486000, 110.4421000],
    [-7.0489452, 110.4425284], // Halte FKM

    // === FKM → FPIK (Jl Prof Jacub Rais, ke selatan) ===
    [-7.0496000, 110.4423000],
    [-7.0502000, 110.4421000],
    [-7.0507341, 110.4420429], // Halte FPIK

    // === FPIK → FPP (Jl Prof Jacub Rais, ke selatan) ===
    [-7.0515000, 110.4418000],
    [-7.0522000, 110.4416000],
    [-7.0530393, 110.4412919], // Halte FPP

    // === FPP → FPP After Sapi (Jl Prof Soedarto, ke selatan) ===
    [-7.0536000, 110.4406000],
    [-7.0541000, 110.4401000],
    [-7.0545672, 110.4396236], // Halte FPP After Sapi

    // === FPP After Sapi → Bundaran UNDIP (lurus ke selatan) ===
    [-7.0552000, 110.4395000],
    [-7.0560116, 110.4393959], // Halte Bundaran UNDIP

    // === Bundaran UNDIP → Rusunawa (Jl Prof Soedarto ke timur) ===
    [-7.0559000, 110.4403000],
    [-7.0556000, 110.4415000],
    [-7.0552000, 110.4428000],
    [-7.0548000, 110.4436000],
    [-7.0545246, 110.4441404], // Halte Rusunawa

    // === Rusunawa → Joging Track (kembali ke barat/utara) ===
    [-7.0548000, 110.4430000],
    [-7.0551000, 110.4415000],
    [-7.0553000, 110.4405000],
    [-7.0553551, 110.4392964], // Halte Joging Track
];
...
async function fetchOSRMSegment(from, to) {
    // OSRM public API — gratis, tanpa API key, ikut jalan nyata
    ...
    try {
        ...
        if (data.code === "Ok" && data.routes && data.routes[0]) {
            // GeoJSON coords adalah [lng, lat] — kita balik jadi [lat, lng] untuk Leaflet
...
async function drawRoutePolylines() {
    ...
    // Ambil rute segmen per segmen (titik ke titik berikutnya)
    // OSRM tidak punya batasan jumlah titik, tapi kita request per-segmen
    // agar lebih akurat mengikuti jalan dan mudah di-debug
    const allCoords = ROUTE_1_PATH;
    const allRoutePoints = [];
    let osrmBerhasil = false;

    for (let i = 0; i < allCoords.length - 1; i++) {
        const from = allCoords[i];
        const to   = allCoords[i + 1];
        const pts  = await fetchOSRMSegment(from, to);

        if (pts && pts.length > 0) {
            // Hindari duplikasi titik sambungan antar segmen
            if (allRoutePoints.length > 0) pts.shift();
            allRoutePoints.push(...pts);
            osrmBerhasil = true;
        } else {
            // Kalau segmen ini gagal, pakai garis lurus sebagai fallback segmen
            if (allRoutePoints.length > 0) allRoutePoints.push(to);
            else allRoutePoints.push(from, to);
        }
    }

    if (osrmBerhasil && allRoutePoints.length > 0) {
        L.polyline(allRoutePoints, {
            color: '#3b82f6', weight: 4, opacity: 0.85, lineCap: 'round', lineJoin: 'round'
        }).addTo(map);
    } else {
        // Fallback total — garis putus-putus kalau semua OSRM gagal
