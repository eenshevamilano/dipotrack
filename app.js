...
async function drawRoutePolylines() {
    ...
    for (let i = 0; i < allCoords.length - 1; i += chunkSize - 1) {
        ...
        try {
            ...
            // skip chunk ini, lanjut ke berikutnya
        }
    }
    ...
    if (allRoutePoints.length > 0) {
        ...
        // Fallback ke OSRM kalau GraphHopper gagal
        try {
            const coords = ROUTE_1_PATH.map(c => `${c[1]},${c[0]}`).join(';');
            ...
            if (data.routes && data.routes[0]) {
                ...
                L.polyline(routeCoords, {
                    color: '#3b82f6', weight: 4, opacity: 0.75, lineCap: 'round', lineJoin: 'round'
                }).addTo(map);
            }
        } catch (e) {
