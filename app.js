...
// Waypoint manual mengikuti jalan kampus UNDIP
// (titik-titik tambahan di antara halte agar garis tidak lurus/nyasar keluar kampus)
const ROUTE_1_PATH = [
    // Joging Track → Baskoro (lurus ke utara)
    [-7.0553551, 110.4392964],
    [-7.0536302, 110.4391998],
    // Baskoro → FISIP Hukum (belok kiri ke barat)
    [-7.0521000, 110.4391000],
    [-7.0509097, 110.4371935],
    // FISIP Hukum → FIB Vokasi (lurus ke utara sedikit)
    [-7.0504200, 110.4360992],
    // FIB Vokasi → Widpur (belok kanan ke timur)
    [-7.0498503, 110.4384971],
    // Widpur → Samwa (lurus ke utara)
    [-7.0487483, 110.4402190],
    // Samwa → Psiko (belok kiri ke barat laut)
    [-7.0471884, 110.4387009],
    // Psiko → FEB (belok kanan ke timur)
    [-7.0476915, 110.4410277],
    // FEB → FKM (lurus ke timur)
    [-7.0489452, 110.4425284],
    // FKM → FPIK (lurus ke selatan)
    [-7.0507341, 110.4420429],
    // FPIK → FPP (lurus ke selatan)
    [-7.0530393, 110.4412919],
    // FPP → FPP After Sapi (lurus ke selatan)
    [-7.0545672, 110.4396236],
    // FPP After Sapi → Bundaran UNDIP (lurus ke selatan)
    [-7.0560116, 110.4393959],
    // Bundaran UNDIP → Rusunawa (belok kiri ke timur)
    [-7.0558000, 110.4420000],
    [-7.0545246, 110.4441404],
    // Rusunawa → Joging Track (balik ke barat lalu utara)
    [-7.0553000, 110.4420000],
    [-7.0553551, 110.4392964],
];
...
function drawRoutePolylines() {
    // Gambar rute manual mengikuti jalan kampus
