...
// Rute manual berdasarkan jalan kampus UNDIP:
// JT→Baskoro: Jl Prof Eko Budiharjo (ke utara)
// Baskoro→FISIP: Jl Prof Eko Budiharjo lanjut ke Jl Dr Antonius Suroyo (ke barat laut)
// FISIP→FIB: Jl Dr Antonius Suroyo (ke barat)
// FIB→Widpur: Jl Antonius Suroyo (ke timur laut)
// Widpur→Samwa: Jl Prof Soedarto (ke timur)
// Samwa→Psiko: Jl Prof Mr Sunario (ke utara)
// Psiko→FEB: Jl Prof Mr Sunario (balik ke selatan)
// FEB→FKM: Jl Prof Jacub Rais (ke timur)
// FKM→FPP: Jl Prof Jacub Rais (ke selatan)
// FPP→FPP2→Bundaran→Rusunawa: Jl Prof Soedarto
const ROUTE_1_PATH = [
    // === Joging Track → Baskoro (Jl Prof Eko Budiharjo, lurus ke utara) ===
    [-7.0553551, 110.4392964], // Halte Joging Track
    [-7.0547000, 110.4392500],
    [-7.0541000, 110.4392200],
    [-7.0536302, 110.4391998], // Halte Baskoro

    // === Baskoro → FISIP Hukum (Jl Prof Eko Budiharjo → belok kiri ke Jl Dr Antonius Suroyo) ===
    [-7.0530000, 110.4391500],
    [-7.0522000, 110.4388000],
    [-7.0515000, 110.4382000],
    [-7.0509097, 110.4371935], // Halte FISIP Hukum

    // === FISIP Hukum → FIB Vokasi (Jl Dr Antonius Suroyo, ke barat) ===
    [-7.0506000, 110.4367000],
    [-7.0504200, 110.4360992], // Halte FIB Vokasi

    // === FIB Vokasi → Widpur (Jl Antonius Suroyo, ke timur laut) ===
    [-7.0502000, 110.4365000],
    [-7.0500000, 110.4372000],
    [-7.0498503, 110.4384971], // Halte Widpur

    // === Widpur → Samwa (Jl Prof Soedarto, ke timur) ===
    [-7.0492000, 110.4392000],
    [-7.0487483, 110.4402190], // Halte Samwa

    // === Samwa → Psiko (Jl Prof Mr Sunario, ke utara/barat laut) ===
    [-7.0481000, 110.4397000],
    [-7.0475000, 110.4391000],
    [-7.0471884, 110.4387009], // Halte Psiko

    // === Psiko → FEB (Jl Prof Mr Sunario, balik ke timur menuju FEB) ===
    [-7.0473000, 110.4394000],
    [-7.0475000, 110.4401000],
    [-7.0476915, 110.4410277], // Halte FEB

    // === FEB → FKM (Jl Prof Jacub Rais, ke timur) ===
    [-7.0482000, 110.4416000],
    [-7.0489452, 110.4425284], // Halte FKM

    // === FKM → FPIK (Jl Prof Jacub Rais, ke selatan) ===
    [-7.0497000, 110.4423000],
    [-7.0507341, 110.4420429], // Halte FPIK

    // === FPIK → FPP (Jl Prof Jacub Rais, ke selatan) ===
    [-7.0518000, 110.4417000],
    [-7.0530393, 110.4412919], // Halte FPP

    // === FPP → FPP After Sapi (Jl Prof Soedarto, ke selatan) ===
    [-7.0537000, 110.4405000],
    [-7.0545672, 110.4396236], // Halte FPP After Sapi

    // === FPP After Sapi → Bundaran UNDIP (Jl Prof Soedarto, ke selatan) ===
    [-7.0552000, 110.4395000],
    [-7.0560116, 110.4393959], // Halte Bundaran UNDIP

    // === Bundaran UNDIP → Rusunawa (Jl Prof Soedarto, ke timur) ===
    [-7.0558000, 110.4405000],
    [-7.0553000, 110.4418000],
    [-7.0545246, 110.4441404], // Halte Rusunawa

    // === Rusunawa → Joging Track (kembali ke barat/utara) ===
    [-7.0549000, 110.4425000],
    [-7.0551000, 110.4410000],
    [-7.0553551, 110.4392964], // Halte Joging Track
];
...
async function drawRoutePolylines() {
    ...
    // Bagi jadi segmen-segmen kecil (max 5 titik per request GraphHopper free)
    const allCoords = ROUTE_1_PATH;
    const chunkSize = 5;
    ...
    for (let i = 0; i < allCoords.length - 1; i += chunkSize - 1) {
        ...
        const pointParams = chunk.map(c => `point=${c[0]},${c[1]}`).join('&');
        const url = `https://graphhopper.com/api/1/route?${pointParams}&vehicle=car&points_encoded=false&key=LijBPDQGfu7Iiq80w3HzwB4RUDJbMbj78nz`;

        try {
            const res = await fetch(url);
            const data = await res.json();
            if (data.paths && data.paths[0]) {
                const pts = data.paths[0].points.coordinates.map(c => [c[1], c[0]]);
                allRoutePoints.push(...pts);
            }
        } catch (e) {
            // skip chunk ini
        }
    ...
    if (allRoutePoints.length > 0) {
        L.polyline(allRoutePoints, {
            color: '#3b82f6', weight: 4, opacity: 0.8, lineCap: 'round', lineJoin: 'round'
        }).addTo(map);
    } else {
        // Fallback garis lurus kalau GraphHopper gagal
        L.polyline(ROUTE_1_PATH, {
            color: '#3b82f6', weight: 4, opacity: 0.6, dashArray: '8,8', lineCap: 'round'
        }).addTo(map);
    }
}
